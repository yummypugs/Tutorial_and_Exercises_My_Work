...
For this tutorial, let us use [Ames Housing dataset](https://www.kaggle.com/competitions/house-prices-advanced-regression-techniques/data) on Kaggle to explore the housing prices. One simpliest way to model housing prices is a "multiple" measure between between housing area and price:
...
The second derivative gives us indication that the loss function is convex. So we can set first derivative to zero to get the global minumum.
...
On your exercise, the matrix form is used to accomodate intercept term and more features. For example, if we add an intercept to our equation:
...
To show that the loss function is convex you need to show that the second derivative of the function, known as the Hessian matirx, is positive semi-definite. This step can be skipped. Set the Jacobian matrix to 0, you will get the normal equation.
